class Activity < ApplicationRecord
  ...
  def self.log_team_member_added(user, team, member, ip_address: nil, user_agent: nil)
    create!(
      user: user,
      company: user.company,
    ...
    Rails.logger.error "Failed to log team member addition activity: #{e.message}"
  end

  def self.log_team_member_removed(user, removed_user:, ip_address: nil, user_agent: nil)
    create!(
      user: user,
      company: user.company,
      action: 'team_member_removed',
      resource_type: 'Team',
      resource_id: user.team&.id,
      ip_address: ip_address,
      user_agent: user_agent,
      metadata: { 
    ...
    Rails.logger.error "Failed to log team member removal activity: #{e.message}"
  end

  def self.log_team_invitation(user, invited_email:, role:, ip_address: nil, user_agent: nil)
    create!(
      user: user,
      company: user.company,
    ...
    Rails.logger.error "Failed to log team invitation activity: #{e.message}"
  end

  def self.log_team_ownership_transferred(user, new_owner:, ip_address: nil, user_agent: nil)
    create!(
      user: user,
      company: user.company,
    ...
    Rails.logger.error "Failed to log team ownership transfer activity: #{e.message}"
  end
